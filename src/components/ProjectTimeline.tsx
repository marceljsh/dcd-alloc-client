...
const getTaskPosition = (
  ...
  const startIndex = dailyHeaders.findIndex(
    (d) => d.toDateString() === new Date(task.startDate).toDateString(),
  );
  const endIndex = dailyHeaders.findIndex(
    (d) => d.toDateString() === new Date(task.endDate).toDateString(),
  );
  const left = (startIndex >= 0 ? startIndex : 0) * dayWidth;
  const width =
    ((endIndex >= 0 ? endIndex : dailyHeaders.length - 1) -
    ...
    while (true) {
      const overlap = sorted.some((other) => {
        ...
        const s = new Date(task.startDate),
          e = new Date(task.endDate);
        const os = new Date(other.startDate),
          oe = new Date(other.endDate);
        return s <= oe && e >= os;
      });
      if (!overlap) break;
      level++;
...
const generateDummyProject = (): Project => {
  ...
  const stages: Stage[] = stageNames.map((stageName, stageIndex) => {
    ...
    const tasks: Task[] = Array.from({ length: numTasks }, (_, i) => {
      ...
      const taskName =
        taskNamePool[Math.floor(Math.random() * taskNamePool.length)];
      return {
        id: taskId++,
        name: taskName,
...
export default function ProjectTimeline() {
  ...
  const monthHeaders = useMemo(() => {
    ...
    dailyHeaders.forEach((d) => {
      ...
      const currentMonthName = d.toLocaleString("en-US", {
        month: "long",
        year: "numeric",
      });
          ...
          <TooltipProvider>
            {project.stages.map((stage) => {
              ...
              const rowHeight = Math.max(
                64,
                (Math.max(...Object.values(taskLevels), 0) + 1) * 36 + 16,
              );
                          ...
                          <Tooltip key={task.id}>
                            ...
                            <TooltipContent side="top" className="max-w-xs">
                              <div className="space-y-1">
                                ...
                                <div className="text-sm flex gap-2 items-center">
                                  <Zap className="h-3 w-3" /> {task.storyPoints}{" "}
                                  story points
                                </div>
