...
const tasks: Dict<number, Task> = new Dict([
  [1,  { id: 1,  stageId: 1, employeeId: 1, name: 'Requirements Gathering', startDate: '2024-01-03', endDate: '2024-01-10', storyPoints: 8 }],
  [2,  { id: 2,  stageId: 1, employeeId: 6, name: 'System Design', startDate: '2024-01-08', endDate: '2024-01-20', storyPoints: 13 }],
  [3,  { id: 3,  stageId: 2, employeeId: 2, name: 'Database Schema', startDate: '2024-01-15', endDate: '2024-01-25', storyPoints: 5 }],
  ...
  [10, { id: 10, stageId: 3, employeeId: 3, name: 'Code Review', startDate: '2024-02-03', endDate: '2024-02-07', storyPoints: 3 }],
  [11, { id: 11, stageId: 6, employeeId: 4, name: 'Testing Setup', startDate: '2024-02-05', endDate: '2024-02-26', storyPoints: 8 }],
])
...
const taskColors = [
  'bg-red-500',    'bg-blue-500',    'bg-green-500', 'bg-yellow-500', 'bg-purple-500',  'bg-teal-500',
  'bg-orange-500', 'bg-indigo-500',  'bg-lime-500',  'bg-rose-500',   'bg-cyan-500',    'bg-amber-500',
  'bg-violet-500', 'bg-emerald-500', 'bg-pink-500',  'bg-sky-500',    'bg-fuchsia-500', 'bg-slate-500',
] as const

const getTaskColor = (index: number) => taskColors[index % taskColors.length]

export default function Component({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  ...
  const weeklyHeaders = useMemo(() => {
    ...
    while (current <= endDate) {
      ...
      const daysInWeek = dailyHeaders
        .filter((day) => day >= current && day <= weekEnd)
        .length
  ...
  const getTaskPosition = (task: Task) => {
    ...
    const actualStartIndex = startDayIndex >= 0
        ? startDayIndex
        : Math.max(
            0,
            Math.floor(
    ...
    const actualEndIndex = endDayIndex >= 0
        ? endDayIndex
        : Math.min(
  ...
  return (
    <div className={cn('space-y-6', className)} {...props}>
            ...
            <div className="relative overflow-hidden">
              ...
              <div className="flex flex-col max-h-[600px]">
                ...
                <div className="flex-grow overflow-auto">
                  ...
                  <div style={{ minWidth: `${employeeColumnFixedW + employeeColumnWidth}px` }}>
                            ...
                            <div className="flex-1 relative">
                              ...
                              <div className="relative p-2">
                                {employeeTasks.map((task) => {
                                  ...
                                  const taskStart = new Date(task.startDate).toLocaleDateString()
                                  const taskEnd = new Date(task.endDate).toLocaleDateString()
                                      ...
                                      <TooltipTrigger asChild>
                                        <div
                                          className={cn(
                                            ...
                                            getTaskColor(projects.getKeys().indexOf(project.id)),
                                          )}
                                          style={{
                                            left: position.left,
      ...
      <div className="grid gap-4 mx-auto [grid-template-columns:repeat(auto-fit,minmax(200px,1fr))]">
        {projects.getValues().filter((p) => selectedProjects.includes(p.id)).map((project) => {
          const stageData = stages.getValues().filter(stage => stage.projectId === project.id)
            .map(stage => {
              const tasksInStage = tasks.getValues().filter(task => task.stageId === stage.id)
              return {
                ...stage,
                tasks: tasksInStage,
              }
          ...
          const projectColor = getTaskColor(projects.getKeys().indexOf(project.id))
                ...
                <div className="space-y-2 text-sm">
                  {stageData.map(stage => (
                    <div key={stage.id} className="font-medium">
                      {stage.name}: <span className="font-normal">{stage.tasks.map(task => task.name).join(', ')}</span>
                    </div>
                  ))}
                </div>
              </CardContent>
